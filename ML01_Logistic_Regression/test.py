...
def logistic_regression(X_train, y_train, alpha=0.01, num_iterations=1000):
...
def logistic_regression(X_train, y_train, alpha=0.01, max_iter=1000, tol=1e-5):
    m, n = X_train.shape
    n_classes = y_train.shape[1]
    theta = np.zeros((n + 1, n_classes))  # Adding one for bias
    X_train_with_bias = np.hstack((np.ones((m, 1)), X_train))  # Add bias term to features
...
# The logistic regression in sklearn can handle multi-class but expects single column labels,
# so no need for label binarization unless you're doing something specific with the predictions.
...
# Since the actual y_test is not binarized, for ROC plotting, you would need to binarize it.
y_test_binarized = label_binarize(y_test, classes=[0, 1, 2])

# Plot ROC curve
roc_plot(y_score, y_test_binarized)'''
